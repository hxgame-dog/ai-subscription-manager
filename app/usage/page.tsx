    ...
    <div className="page-stack">
      ...
      <section className="card">
        <div className="section-head">
          <div>
            <h2>手动同步</h2>
            <p>先诊断、再同步，再回看任务、用量和费用记录。这里是整个同步链路的操作入口。</p>
          </div>
        </div>
          ...
          <div className="soft-card">
            {readyProviders.length > 0 ? (
              ...
              <>
                ...
                <span>目前自动同步平台里还没有 ready 的真实 connector，建议先完成 Cursor、Gemini 或 OpenAI。</span>
              </>
            )}
          </div>
        </section>
        ...
        {latestJob ? (
          <section className="card">
            ...
            <div className="page-block">
              <p>
                <strong>{latestJob.provider?.name || "ALL"}</strong> ·{" "}
                <span className={`badge ${latestJob.status === "SUCCESS" ? "success" : latestJob.status === "FAILED" ? "danger" : "info"}`}>
                  {latestJob.status}
                </span>
              </p>
              <p>
                触发方式：{latestJob.trigger} · 时间窗口：{formatWindow(latestJobWindow)} · 运行时长：
                {formatDuration(latestJob.startedAt, latestJob.finishedAt)}
              </p>
              ...
              {jobs.map((j) => {
                ...
                return (
                  <tr key={j.id}>
                    ...
                    <td>
                      <div>
                        {j.trigger} · {formatWindow(windowDays)}
                      </div>
